...
<p align="center">
  <img src="internal/mcp/webui/logo-transparent.png" alt="jamypg Logo" width="180" />
</p>

Go-based MCP server for metadata-grounded NL2SQL over **PostgreSQL, MySQL, and
MariaDB**. Current source version: `v0.35.0` (converted from the Oracle-based
jasql project).
...
| 컨테이너 (모든 DB 실행 가능) | `docker build -t jamypg-mcp:v0.35.0 .` |
...
```sh
docker build -t jamypg-mcp:v0.35.0 .
...
### DBA 관리 도구 (privileged, `dba`/`admin` 역할 전용)

프로파일에 **DBA 자격증명**(`db_profiles`의 `dba.enabled`+`dba.username`+`dba.password_ref`)을 설정한 경우에만 사용할 수 있습니다. 읽기 전용 쿼리 계정과 **분리된 쓰기 가능 커넥션**으로 실행되며, 모든 변경은 감사 로그(`dba:*`)에 기록됩니다. 관리 화면: `/admin/dba-console`.
...
- `dba_create_user` — 사용자/역할 생성(postgres LOGIN/SUPERUSER/CREATEDB/CREATEROLE, mysql `CREATE USER`). 비밀번호는 감사 로그에서 마스킹
- `dba_alter_user` — 비밀번호·속성 변경
- `dba_drop_user` — 사용자/역할 삭제(`confirm=true` 필수)
- `dba_grant` — 권한 부여/회수(`revoke=true`), `WITH GRANT OPTION` 지원
- `dba_create_database` — 데이터베이스 생성(OWNER/ENCODING 또는 CHARACTER SET)
- `dba_drop_database` — 데이터베이스 삭제(`confirm=true` 필수)
- `dba_set_parameter` — 설정 변경(postgres `ALTER SYSTEM`+`pg_reload_conf()`, mysql `SET GLOBAL/SESSION`)
- `dba_terminate_session` — 세션 종료/쿼리 취소(`pg_terminate_backend`/`pg_cancel_backend`, mysql `KILL`)
- `dba_run_maintenance` — 유지보수(postgres VACUUM/ANALYZE/REINDEX, mysql ANALYZE/OPTIMIZE)
- `dba_execute` — 임의 권한 SQL 실행(에스케이프 해치, `confirm=true` 필수, 감사 로그에 원문 기록)
...
- `export_lineage_to_openmetadata` — jamypg 관계 그래프를 OpenMetadata **테이블 lineage 엣지**로 push(from=참조/부모, to=기준/자식). FK 관계형 lineage 매핑(ETL 흐름 아님). `dry_run=true` 계획(기본)/`false` 반영(관리자), OM에 없는 테이블 엣지는 skip 보고
...
`JAMYPG_FEEDBACK_TENANT`를 서버가 관리하는 고정 값으로 설정하세요.
